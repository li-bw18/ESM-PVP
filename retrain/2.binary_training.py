...
def trail(model, train_dataloader, val_dataloader, criterion, optimizer, nepoch, path):
    ...
    for epoch_id in range(nepoch):
        ...
        for batch_id, batch_data in enumerate(train_dataloader):
            ...
            predicts = F.softmax(model(inputs)[4], 1)
        ...
        if valid_auc > current_highest_auc:
            current_highest_auc = valid_auc
        ...
        if epoch_id - current_best_epoch == 2:
            break
    print("Best Valid AUC:", current_highest_auc)  
...
train_loss_list, valid_loss_list, valid_auc_list = trail(esmpvp, train_dataloader, val_dataloader, criterion, optimizer, nepoch, "model/binary.pth")
